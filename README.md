...
관리자 로그인 → **지침서 관리 → BM25 기준선 평가 · 관리자 전용**에서 평가할 문서와 질문을 선택합니다.
이 기능은 저장된 chunk로 BM25만 실행하며 vector 검색과 LLM을 호출하지 않습니다.
결과는 서버의 `artifacts/bm25_results.csv`, `artifacts/bm25_results.json`에 저장되고 같은 화면에서 내려받을 수 있습니다.
두 파일에는 지침 원문이 포함되므로 Git에 추가하거나 외부에 공유하지 마세요.
score가 0인 chunk는 원시 순위 확인용이며 검색 성공으로 판정하지 않습니다.
...
- 관리자 채팅 답변 아래의 BM25 원시 검색 결과: 실제 query, 점수, 문서명, 페이지, section, 원문 앞 300자
- BM25 top 10 CSV 저장: query, rank, bm25_score, 문서·페이지·section·chunk ID와 전체 chunk 원문
...
PDF의 한 문장이 지면 너비 때문에 여러 줄로 나뉜 경우, 명확한 한국어 연결어/조사 뒤의 줄바꿈을 공백으로 해석합니다.
따라서 원문의 완전한 문장을 한 줄로 인용해도 잘못 거절하지 않으며, 해당 문장의 한 줄 조각만 인용하면 거절합니다.
빈 줄·표 행·목록·항목 경계와 단어·수치·단위·조건·부정은 유지합니다. 불명확한 줄바꿈을 임의로 합치지 않습니다.
본문 없이 문서 제목·항목명·그림 예시 제목만 있는 chunk는 벡터 유사도가 높아도 검색 seed와 답변 근거에서 제외합니다.
이 변경은 검색/인용 검증 단계에 적용되므로 저장 원문과 기존 chunk·벡터를 수정하지 않습니다.
