...
from src.diagnostic_ui import render_diagnostics
from src.documents import MAX_FILE_MB, PdfInputError, read_document
from src.library import CHUNK_VERSION, protect_private
from src.operations import create_backup, verify_backup
from src.pdf_layout import EXTRACTION_VERSION, ocr_status
from src.settings import GuideError
from src.ui import document_summary
...
def render_admin(library, auth, settings, model_factory):
    ...
    with st.expander("관리자 설정 안내 / 직원 계정"):
        st.write("설정 파일: src/.env · API 키는 화면이나 GitHub에 올리지 마세요.")
        ...
        st.caption("설치 후 src/.env의 GUIDE_OCR_ENABLED=true로 활성화하고 문서를 재색인하세요.")
        ...
        if isinstance(auth, LocalAuth):
            ...
            st.caption("자세한 SQL·Storage 설정은 프로젝트 README.md에 있습니다.")
