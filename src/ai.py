...
from src.library import NO_GUIDELINE, clean, protect_private
from src.presentation import AnswerPresentation, build_answer_presentation
from src.settings import ROOT, GuideError

if TYPE_CHECKING:
    from src.evidence import RequiredFacet
...
def validate_answer(raw, hits, trace=None, plan=None):
    from src.evidence import procedure_citations_ordered, sentence_evidence
    ...
    try:
        ...
        for statement in result.statements:
            ...
            from src.grounding import unsupported_action
...
def estimated_tokens(messages, provider, output_limit=OUTPUT_LIMIT):
...
def estimated_request_tokens(messages, provider, contract=None, output_limit=OUTPUT_LIMIT):
    """Reserve prompt, completion, and Groq response-schema tokens conservatively."""
    reserved = estimated_tokens(messages, provider, output_limit)
...
def _gemini_openai_compatible(settings, endpoint):
    return (
        settings.llm_provider == "internal"
        and settings.llm_model.startswith("gemini-")
        and urlparse(endpoint).hostname == "generativelanguage.googleapis.com"
    )
...
def prompt_evidence_envelope(groups, catalog=None, contract=None, requirement=None):
    from src.evidence import build_source_unit_catalog
...
def _validate_group_source_unit_selection(
    ...
    from src.evidence import answer_coverage
    try:
...
def generate(settings, question, hits, user_id, quota=None, transport=None, plan=None, trace=None):
    ...
    if len(prompt_result) == 4:
        ...
        from src.evidence import build_source_unit_catalog, evidence_groups
    ...
    from src.library import anchors
    for h in selected:
    ...
    gemini_compatible = _gemini_openai_compatible(settings, endpoint)
    output_limit = GEMINI_OUTPUT_LIMIT if gemini_compatible else OUTPUT_LIMIT
    reserved = estimated_request_tokens(
        messages, settings.llm_provider, selection_contract, output_limit,
    )
    if settings.llm_provider == 'groq_free':
    ...
    if settings.llm_provider == "groq_free" or gemini_compatible:
        payload["reasoning_effort"] = "low"
    headers = {"Content-Type": "application/json"}
    if settings.llm_key:
        headers["Authorization"] = "Bearer " + settings.llm_key
    try:
        # 일시적인 internal provider 503만 한 번 재시도합니다. 모델 전환·웹 검색·추적
        # 서비스 전송은 하지 않습니다.
        if trace is not None:
            trace.update(
                llm_called=True,
                stage='llm_request',
                response_parse_stage='transport',
                provider_retry_count=0,
            )
        with httpx.Client(timeout=30, transport=transport, follow_redirects=False) as client:
            response = client.post(endpoint, json=payload, headers=headers)
            if settings.llm_provider == "internal" and response.status_code == 503:
                if trace is not None:
                    trace.update(provider_first_http_status=503, provider_retry_count=1)
                time.sleep(0.5)
                response = client.post(endpoint, json=payload, headers=headers)
        if trace is not None:
        ...
        if response.status_code == 503:
            try:
                ...
                pass
            raise GuideError(
                "AI 서버가 일시적으로 혼잡합니다. 잠시 후 같은 질문을 다시 시도해 주세요. "
                "(AI_UNAVAILABLE)"
            )
