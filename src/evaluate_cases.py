...
실행: python -m src.evaluate_cases [--live] [--case exact]
실제 병원 지침서/계정/질문을 읽지 않으며 결과는 workspace/RAG_실험에 저장합니다.
"""
...
from src.library import Chunk, Embedder, LocalLibrary, bounded_embedding_question
...
def main():
    ...
    destination = ROOT / 'workspace' / 'RAG_실험' / ('rag-live-' + (args.case or 'all') + '.json' if args.live else 'rag-10-cases.json')
