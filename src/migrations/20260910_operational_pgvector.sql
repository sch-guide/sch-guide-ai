...
alter table public.guide_documents add column if not exists extraction_version integer not null default 1;
alter table public.guide_documents add column if not exists chunk_version integer not null default 1;
alter table public.guide_documents add column if not exists chunk_quality jsonb;
alter table public.guide_documents add column if not exists warnings jsonb not null default '[]'::jsonb;
alter table public.guide_documents add column if not exists missing_locations jsonb not null default '[]'::jsonb;
...
begin
    ...
    insert into public.guide_documents(
        ...
        source_type,chunk_count,indexed_at,last_error,replaces_id,extraction_version,
        chunk_version,chunk_quality,warnings,missing_locations
    ) values (
        ...
        coalesce((doc->>'extraction_version')::integer,1),
        coalesce((doc->>'chunk_version')::integer,1),doc->'chunk_quality',
        coalesce(doc->'warnings','[]'::jsonb),coalesce(doc->'missing_locations','[]'::jsonb)
    ) returning * into d;
...
begin
    ...
    update public.guide_documents set
        ...
        extraction_version=coalesce((doc->>'extraction_version')::integer,extraction_version),
        chunk_version=coalesce((doc->>'chunk_version')::integer,chunk_version),
        chunk_quality=doc->'chunk_quality',warnings=coalesce(doc->'warnings',warnings),
        missing_locations=coalesce(doc->'missing_locations',missing_locations)
    where id=target;
